...
class PlanModel {
    ...
    async getPlanById(idplan) {
        try {
            const { rows } = await pool.query('SELECT * FROM plan WHERE idplan = ?', [idplan]);
            return rows[0];
        } catch (error) {
            throw error;
        }
    ...
    async updatePlan(idplan, planData) {
        try {
            ...
            const { rowCount } = await pool.query('UPDATE plan SET dateplan = ? WHERE id = ?', [dateplan, idplan]);
            return rowCount > 0;
        } catch (error) {
            throw error;
        }
    ...
    async deleteUser(idplan) {
        try {
            const {rowCount} = await pool.query('DELETE FROM plan WHERE idplan = ?', idplan);
            return rowCount > 0;
        }
        catch (error) {
            throw error;
